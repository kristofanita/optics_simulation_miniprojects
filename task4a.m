...
% Generate parallel rays before the lens
Ez_before_lens = waveamp(size(Y, 1), size(X, 2), size(Y, 1) / 2, 1, 1, x(2) - x(1), y(2) - y(1), lambda);
...
% Plot the parallel rays before the lens
for i = 1:20:length(x)
    plot([-source_distance, 0], [y(i), y(i)], 'r--', 'HandleVisibility', 'off');
end
...
title('Light paths through a lens');
xlabel('x (m)');
ylabel('y (m)');
legend;
grid on;
